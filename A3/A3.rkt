...
(define-syntax all-helper
  (syntax-parser
    [(_ e1 es ...)
     #'(let ([eval e1])
         (if eval
             (cons eval (all-helper es ...))
             (cons #f empty)))]
    [(_)
     #'empty]))
...
(define-syntax all
  (syntax-parser
    [(_ es ...)
     #'(let ([output (all-helper es ...)])
         (if (memv #f output)
             #f
             (filter (lambda (x) (not (eqv? #t x))) output)))]))
...
(check-equal? (all #t 1 #t 2 #t 3) '(1 2 3))
(check-equal? (all 1 2 3 4 5 6 7 8 9 #f) #f)
(check-equal? (all 1 2 3 4 5 #t #f 6 7 8 9 #t #f) #f)
; (check-equal? (all (print 'hi) #f (print 'hello)) #f)
...
(struct/con alist [(index : number?) (val : boolean?)])
(define a (alist 1 #f))
(check-equal? (alist-index a) 1)
(check-equal? (alist-val a) #f)

(struct/con pixel [(x : number?) (y : number?) (color : string?)])
(define px (pixel 140 200 "red"))
(check-equal? (pixel-x px) 140)
(check-equal? (pixel-color px) "red")